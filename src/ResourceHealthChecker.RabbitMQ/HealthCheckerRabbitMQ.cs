...
namespace SlugEnt.ResourceHealthChecker.RabbitMQ
{
    ...
    public class HealthCheckerRabbitMQ : AbstractHealthChecker
    {
         ...
         descriptiveName, EnumHealthCheckerType.RabbitMQ, mqConfig, logger)
        {
            ...
            {
                _logger.LogCritical("Health Checker {HealthChecker} constructor encountered configuration issues.  Health Check will not run.  Error: {ErrorMsg}",
                                    ShortTitle, ex.Message);
            }
        }
        ...
        protected void ConstructURL()
        {
            ...
            if (MQConfig.User == string.Empty)
                throw new ArgumentException(errorMsg + "No Username provided in the config");
            if (MQConfig.Password == string.Empty)
                throw new ArgumentException(errorMsg + "No Password provided in the config");
            if (MQConfig.ServerIP == string.Empty)
                throw new ArgumentException(errorMsg + "No Server IP provided in the config");
            if (MQConfig.Instance == string.Empty)
                throw new ArgumentException(errorMsg + "No Instance name provided in the config");
        }
        ...
        protected void DeConstructURL()
        {
            ...
            if (urlLower.StartsWith("amqp"))
                MQConfig.IsEncrypted = false;
            ...
            if (indexUser < 0)
                throw new ArgumentException(errMsgStart + "Could not find username - missing colon");
            ...
            if (indexPassword < 0)
                throw new ArgumentException(errMsgStart + "Could not find password - missing @");
            ...
            if (indexServer < 0)
                throw new ArgumentException(errMsgStart + "Could not find Server - missing slash ");
        ...
        {
            get { return ShortTitle + "  -->  " + MQConfig.Instance; }
        }
